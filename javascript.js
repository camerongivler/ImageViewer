...
$(function() {
    ...
    socket = io.connect('http://spinoza2.disp.duke.edu:8000');
    socket.on('reload', function() {
        background.attr('src', 'image.jpg');
        thumb.img.attr('src', 'image.jpg');
    });
    ...
    background.hammer().on('release', function() {
        if (FEPos !== null && FEPos.auto === 't')
            return;
        bgPos.moved = true;
        numTouches = 0;
    ...
    shutter.hammer().on('release', function(ev) {
        ...
        background.animate({top: '100%', left: '50%', 'margin-left': '-10px', width: '20px', height: '20px'}, {duration: 300 + scale * 40, queue: false, complete: function() {
                background.css({top: bgPos.top, left: bgPos.left, 'margin-left': '0', width: bgPos.width, height: bgPos.height});
            }});
    });
});
